...
#################################### 统计检验 ##################################
data = read.table("Obesity_phage_family_species_Cor_reshape.txt", header = TRUE, sep = "\t", row.names = 1, stringsAsFactors = FALSE, check.names = F)
head(data)
...
setwd("C:/Users/huxin/Desktop/Final_result_analysys/14_Virome_Bacteris_Cor/family_species")
...
##### 方差分析及分组求均值
phage_statistic <- phage_family %>% group_by(Virome) %>% anova_test(Cor ~ Group, detailed = T) # 分组统计检验
write.table(phage_statistic, file="phage_family_statistics_anova.txt",sep = "\t",row.names=TRUE, col.names=T, quote = FALSE)

Control_phage_mean <- phage_family  %>% filter(Group == "Control") %>% group_by(Virome) %>% summarise(mean_value = mean(Cor))
colnames(Control_phage_mean) <- c("Virome", "Control_mean")
Obesity_phage_mean <- phage_family  %>% filter(Group == "Obesity") %>% group_by(Virome) %>% summarise(mean_value = mean(Cor))
colnames(Obesity_phage_mean) <- c("Virome", "Obesity_mean")
phage_mean_table <- merge(Control_phage_mean, Obesity_phage_mean, by = "Virome")
write.table(phage_mean_table, file="phage_family_mean.txt", sep = "\t", row.names=TRUE, col.names=T, quote = FALSE)
